...
export default function SnackProgressBar({ count, targetSnacks }) {
  ...
  useEffect(() => {
    ...
    if (targetSnacks < 1 || targetSnacks > targetSnacksDefault) targetSnacks = targetSnacksDefault;
  ...
  return (
    <div className="progress-container">
      <div className="progress-wrapper">
        ...
        <div>
          {percentProgress !== 100 && count === 0 && <p>What will be your first snack today?</p>}

          {percentProgress !== 100 && count === 1 && <p>Great Start!!!</p>}

          {percentProgress !== 100 && count > 1 && <p>Excellent, {count} snacks gone!</p>}

          {percentProgress === 100 && <p>Congratulations Jack, all snacks eaten!!!</p>}
        </div>
      </div>
    </div>
  );
...
const ProgressBar = ({ progressPercentage }) => {
  return (
    <div className="progressbar-outer">
      <div style={{ width: `${progressPercentage}%` }} className="progressbar-inner"></div>
    </div>
  );
};
